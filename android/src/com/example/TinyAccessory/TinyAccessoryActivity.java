...
public class TinyAccessoryActivity extends Activity implements Runnable {
	...
	private static final int MESSAGE_ARDUINO = 4;
	...
	protected class ArduinoMsg {
		private byte[] arduino;

		public ArduinoMsg(byte[] arduino) {
			this.arduino = arduino;
		}

		public byte[] getArduino() {
			return arduino;
		}
	}
	...
	public void run() {
		...
		while (ret >= 0) {
			...
			while (i < ret) {
				...
				case (byte) 0xFF:
					if (len >= 7) {
						Message m = Message.obtain(mHandler, MESSAGE_ARDUINO); // look at mhandler delclaration at line 330
						byte temp[] = {buffer[i+1], buffer[i+2], buffer[i+3], buffer[i+4], buffer[i+5], buffer[i+6]};
						m.obj = new ArduinoMsg(temp); //Puts in the message an object that's a new ArduinoMsg class (declared at line 97) that takes in the temp[] buffer shown in the line above
						mHandler.sendMessage(m); //sends this message to the respective method which is  "handleArduinoMessage" in BaseActivity.java
					}
					i += 7;
					break; 
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MESSAGE_ARDUINO:
				ArduinoMsg a = (ArduinoMsg) msg.obj; //Makes sure the object when sent is sent to the right handler method which is "handleArduinoMessage" in BaseActivity.java in this case
				handleArduinoMessage(a);
				break;
